...
\headerbox{$n$-gram model}{name=ngram,column=0,row=0.56,span=2}{%
	One approach to text classification is to develop a \emph{language model}: an approximate probability distribution over word sequences or, in this case, a distribution corresponding to each class. Generically, for a document $X$ consisting of the word sequence $\{w_i\}_{i=1}^n$,
}
...
\headerbox{Results}{name=results,column=2,row=0, span=3}{
	\vspace{-1em}
	\begin{figure}[H]
		\centering
