...
\headerbox{$n$-gram model}{name=ngram,column=0,row=0.38,span=1}{%
	...
	\vspace{-.5em}
	\begin{align*}
		PP(X|Y) &\equiv -\frac{1}{N} \sum_{i=1}^N \log \hat{\P}(w_i|w_{i-n+1}^{i-1},Y).
	\end{align*}
	}


\headerbox{Likelihood-ratio classification}{name=lr,column=1,row=.38}{
	We classified documents based on $n$-gram frequencies in two ways: in \emph{naive Bayes classification}, we classify according to
	\vspace{-.5em}
	\begin{align*}
		\hat{Y} &= \argmax \P(Y) \hat{\P}(X|Y),
	\end{align*}
	%
...
\headerbox{References}{name=refs,column=0,row=0.9,span=2}{}
