...
\section{Implementation} % maybe should be in appendix
...
Here we discuss the theory behind our language models and classifiers; details about our parsing choices can be found in Section~\ref{implementation}.
 
\subsection{Language models}
\subsubsection{$n$-grams}
Let $Y=1$ represent the label ``snarXiv'', let $Y=-1$ represent ``arXiv'', and let $X:=w_1w_2\ldots w_N$ be an abstract containing $N$ words $\{w_i\}_{i=1}^N$.
Assume that each abstract $X$ has a probability $\P(X|Y)$ of being from source $Y\in\{-1,1\}$.

The $n$-grams language model breaks $X$ 
...
The theory behind this classifier is simple: if $\P(-1|X) > \P(1|X)$, then we classify $X$ as arXiv; otherwise, we classify it as snarXiv.
This is an intuitive classification condition -- the only challenge is approximating $\P(Y|X)$ for any abstract $X$.
We can use Bayes' theorem $\P(Y|X) = \frac{\P(Y)\P(X|Y)}{\P(X)}$ to rewrite the NB classification condition as follows:
\begin{align}
	\text{if} \;~ \P(-1)\P(X|-1) > \P(1)\P(X|1): \quad &\text{classify } X \text{ as arXiv} \label{Bayes}
	\\
	\text{else}: \quad &\text{classify } X \text{ as snarXiv}.
\end{align}
...
aka Neyman-Pearson hypothesis test
