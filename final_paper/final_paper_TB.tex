...
In each case, we trained on an equal number $N_\text{train}^Y$ of arXiv and snarXiv abstracts, and we tested on 5000 arXiv abstracts and 5000 snarXiv abstracts.
%
% Add tf-idf stuff?
%

For BoW-NB, we varied $N_\text{train}^Y$ and examined how the classification error changed...
...
We found $\eta_{\text{PP}}=0.7$ to perform best in both models; the true positive rate (TPR) vs. false discovery rate (FDR) is plotted in Figure~\ref{BoW_TPR_FDR}.
We also made histograms of $\frac{\text{PP}(X|snarXiv)}{\text{PP}(X|arXiv)}$, which shows exactly how the ...
