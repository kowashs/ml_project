...
\subsection{$n$-grams} \label{sec:n-grams}
...
We found $\eta_{\text{PP}}=0.7$ to perform best in both models; Figure~\ref{histograms} shows histograms of our estimates for $\frac{\text{PP}(X|\text{snarXiv})}{\text{PP}(X|\text{arXiv})}$ for both the arXiv and snarXiv test corpora.
